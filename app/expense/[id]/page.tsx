...
export default function ExpenseDetailPage() {
  ...
  if (loading) {
    return (
      <div className="mx-auto max-w-3xl space-y-4">
        <Link href="/" className="flex items-center gap-1.5 text-sm text-neutral-500 hover:text-neutral-200">
    ...
    return (
      <div className="mx-auto max-w-3xl space-y-4">
        ...
        <div className="rounded-xl border border-neutral-800 bg-neutral-950 p-8 text-center">
          <p className="text-lg font-semibold text-neutral-300">Expense not found</p>
          <p className="mt-1 text-sm text-neutral-500">
            The expense with ID #{expenseId} does not exist on-chain.
          </p>
          <Link href="/" className="mt-4 inline-flex items-center gap-1.5 text-sm text-neutral-300 hover:text-neutral-100">
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <Link href="/" className="flex items-center gap-1.5 text-sm text-neutral-500 transition-colors hover:text-neutral-200">
        <ArrowLeft size={14} />
        Back to dashboard
      </Link>
      ...
      <div className="rounded-2xl border border-neutral-800 bg-neutral-950 p-6">
        <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
          <h1 className="text-2xl font-bold text-neutral-100 text-balance leading-snug flex-1">
            {expense.description}
          </h1>
          <span
      ...
      <div className="space-y-3">
        <h2 className="text-xs font-semibold uppercase tracking-widest text-neutral-500">Balances</h2>
      ...
      {isParticipant && isOpen && (
        <div className="rounded-xl border border-neutral-800 bg-neutral-950 p-5 space-y-3">
          <h2 className="text-sm font-semibold text-neutral-300">Your Settlement</h2>
          <SettleButton
      ...
      <div className="space-y-3">
        ...
        {settlements.length === 0 ? (
          <p className="rounded-xl border border-dashed border-neutral-800 py-6 text-center text-sm text-neutral-600">
          ...
          <div className="divide-y divide-neutral-800 rounded-xl border border-neutral-800 bg-neutral-950">
            {settlements.map((s, i) => (
              <div key={i} className="flex flex-wrap items-center justify-between gap-2 px-4 py-3">
                ...
                <span className="text-xs text-neutral-600">{formatTimestamp(s.settledAt)}</span>
              </div>
            ))}
          </div>
        )}
