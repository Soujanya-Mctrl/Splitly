...
#[contract]
pub struct SplitlyContract;

#[contractimpl]
impl SplitlyContract {
    ...
    /// Returns the total number of expenses created.
    pub fn get_total_expenses(env: Env) -> u32 {
        env.storage()
            .persistent()
...
mod tests {
    ...
    fn test_create_expense_basic() {
        let env = create_env();
        env.mock_all_auths();
        let contract_id = env.register_contract(None, SplitlyContract);
        let client = SplitlyContractClient::new(&env, &contract_id);
